# Thinking family knowledge dossier — Range Return

**File:** `04_thinking_families/05_return_to_range.md`  
**family_id:** `return_to_range`  
...
## 1. Cognitive purpose
Organize market questions for **Range Return** when family analysis may be available.
...
- `KC-AUCTION-VALUE-AREA` — Session value area (crowd=HIGH)
- `KC-SUPPORT-RESISTANCE` — Support and resistance (crowd=VERY_HIGH)
- `KC-RSI` — Relative Strength Index (crowd=VERY_HIGH)
- `KC-FIB` — Fibonacci retracements (crowd=VERY_HIGH)
- `KC-OPENING-RANGE` — Session opening range (crowd=VERY_HIGH)
...
Queued in `research_hypotheses.json` with `related_families` containing `return_to_range`.
