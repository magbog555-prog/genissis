# Organ knowledge dossier — `cross_symbol_correlation`

**File:** `03_organs/11_correlation.md`  
**Canonical organ_id:** `cross_symbol_correlation`  
**Passport class:** `CROSS_SYMBOL` / `CROSS_CONTEXT_ONLY`  
**Evidence default:** `LITERATURE_CLAIM`  
**Production influence:** `false`  
**RFC-0005:** `NOT OPEN`

## 1. Purpose
Cross-symbol divergence/convergence under explicit context record.

## 2. Market question
Do related symbols diverge/converge under the context record?
...
## 5. Primary data
As per passport evidence_dependencies / Frame slots for `cross_symbol_correlation`.

## 6. Derived data
Only if `CROSS_CONTEXT_ONLY` allows; derived organs never inflate Agreement as extra independent roots.

## 7. Time horizon
Passport `temporal_scope` for `cross_symbol_correlation`.
...
See `algorithm_candidates.json` entries whose organ relevance includes `cross_symbol_correlation` or general research methods.
...
See `06_research_hypotheses/research_hypotheses.json` filtered by `cross_symbol_correlation`.
