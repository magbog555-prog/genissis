# Organ knowledge dossier — `open_interest`

**File:** `03_organs/14_open_interest.md`  
**Canonical organ_id:** `open_interest`  
**Passport class:** `PRIMARY` / `INDEPENDENT_ROOT`  
**Evidence default:** `LITERATURE_CLAIM`  
**Production influence:** `false`  
**RFC-0005:** `NOT OPEN`

## 1. Purpose
OI level/change facts when available.

## 2. Market question
How is OI changing with price when data exists?
...
## 5. Primary data
As per passport evidence_dependencies / Frame slots for `open_interest`.

## 6. Derived data
Only if `INDEPENDENT_ROOT` allows; derived organs never inflate Agreement as extra independent roots.

## 7. Time horizon
Passport `temporal_scope` for `open_interest`.
...
- `KC-FUNDING-EXTREME` — Funding extremes [HIGH/`LITERATURE_ONLY`]
- `KC-OI-PRICE` — Open interest with price [HIGH/`LITERATURE_ONLY`]

## 10. Relevant books (mapped)
- `B132` — Andreas Antonopoulos — Mastering Bitcoin (priority P3)
- `B133` —  — How to DeFi (RU) (priority P3)
- `B134` — Vladimir Popov — ICO: Essence, Problems, Law (RU) (priority P3)
- `B135` — Nathaniel Popper — Digital Gold (RU) (priority P3)
- `B136` — Adam Tepper — Bitcoin and Money (RU) (priority P3)
- `B137` — A. Burenin — Futures, Forwards and Options Markets (RU) (priority P2)
- `B138` — M. Chekulaev — Trading Volatility (alt version, RU) (priority P2)
- `B139` — M. Chekulaev — Trading Volatility (RU) (priority P2)
- `B140` — Kevin Connolly — Buying and Selling Volatility (RU) (priority P2)
- `B141` — A. Feldman — Derivatives: Financial and Commodity (RU) (priority P2)
- `B142` — A. Shvedov — Interest Rate Instruments: Pricing & Hedging (RU) (priority P2)

## 11. Algorithms / formulas found
See `algorithm_candidates.json` entries whose organ relevance includes `open_interest` or general research methods.
...
See `06_research_hypotheses/research_hypotheses.json` filtered by `open_interest`.
