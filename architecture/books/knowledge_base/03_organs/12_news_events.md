# Organ knowledge dossier — `news_events`

**File:** `03_organs/12_news_events.md`  
**Canonical organ_id:** `news_events`  
...
## 1. Purpose
Event windows/tags as external facts — not headline opinions.

## 2. Market question
Is there a bound event window affecting comparability?
...
## 5. Primary data
As per passport evidence_dependencies / Frame slots for `news_events`.

## 6. Derived data
Only if `INDEPENDENT_ROOT` allows; derived organs never inflate Agreement as extra independent roots.

## 7. Time horizon
Passport `temporal_scope` for `news_events`.
...
- `KC-NEWS-IMPULSE` — News impulse [VERY_HIGH/`LITERATURE_ONLY`]
...
See `algorithm_candidates.json` entries whose organ relevance includes `news_events` or general research methods.
...
See `06_research_hypotheses/research_hypotheses.json` filtered by `news_events`.
