# Organ knowledge dossier — `dom_order_book`

**File:** `03_organs/02_dom_order_book.md`  
**Canonical organ_id:** `dom_order_book`  
...
## 1. Purpose
Observe resting book facts (depth, imbalance, replenish) without claiming absorption alone.

## 2. Market question
What resting size geography and imbalance are visible now?
...
## 5. Primary data
As per passport evidence_dependencies / Frame slots for `dom_order_book`.

## 6. Derived data
Only if `INDEPENDENT_ROOT` allows; derived organs never inflate Agreement as extra independent roots.

## 7. Time horizon
Passport `temporal_scope` for `dom_order_book`.
...
- `KC-BOOK-IMBALANCE` — Order book imbalance [MEDIUM/`LITERATURE_ONLY`]
- `KC-ABSORPTION` — Absorption [MEDIUM/`LITERATURE_ONLY`]

## 10. Relevant books (mapped)
- _(no P-mapped sources yet; see gaps)_

## 11. Algorithms / formulas found
See `algorithm_candidates.json` entries whose organ relevance includes `dom_order_book` or general research methods.
...
See `06_research_hypotheses/research_hypotheses.json` filtered by `dom_order_book`.
