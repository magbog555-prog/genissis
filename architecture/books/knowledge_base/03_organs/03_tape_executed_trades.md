# Organ knowledge dossier — `tape_executed_trades`

**File:** `03_organs/03_tape_executed_trades.md`  
**Canonical organ_id:** `tape_executed_trades`  
...
## 1. Purpose
Observe executed aggression and participation as trade facts.

## 2. Market question
What aggression just traded — lift/hit, bursts, participation?
...
## 5. Primary data
As per passport evidence_dependencies / Frame slots for `tape_executed_trades`.

## 6. Derived data
Only if `INDEPENDENT_ROOT` allows; derived organs never inflate Agreement as extra independent roots.

## 7. Time horizon
Passport `temporal_scope` for `tape_executed_trades`.
...
- `KC-VSA-EFFORT-RESULT` — Effort versus result [HIGH/`LITERATURE_ONLY`]
- `KC-WYCKOFF-PHASES` — Wyckoff market phases [HIGH/`LITERATURE_ONLY`]
- `KC-ABSORPTION` — Absorption [MEDIUM/`LITERATURE_ONLY`]
- `KC-BREAKOUT-DOCTRINE` — Breakout doctrine [VERY_HIGH/`LITERATURE_ONLY`]

## 10. Relevant books (mapped)
- `B017` — D. Hutson — Wyckoff Method (RU) (priority P1)
- `B035` — Tom Williams — Master the Markets / VSA (RU) (priority P1)

## 11. Algorithms / formulas found
See `algorithm_candidates.json` entries whose organ relevance includes `tape_executed_trades` or general research methods.
...
See `06_research_hypotheses/research_hypotheses.json` filtered by `tape_executed_trades`.
